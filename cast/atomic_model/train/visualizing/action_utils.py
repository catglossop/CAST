...
from cast.atomic_model.train.visualizing.visualize_utils import (
    to_numpy,
    numpy_to_img,
    VIZ_IMAGE_SIZE,
    RED,
...
def visualize_traj_pred(
    ...
    for i in range(min(batch_size, num_images_preds)):
        ...
        spacing = metric_waypoint_spacing[i].item()

        if normalized:
            pred_waypoints = pred_waypoints * spacing
            label_waypoints = label_waypoints * spacing
...
def plot_trajs_and_points(
    ...
    point_labels: Optional[list] = ["robot"],
