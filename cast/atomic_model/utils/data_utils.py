...
def get_image_paths(data_folder: str, f: str):
    with open(os.path.join(data_folder, f, f"image_paths.txt"), "rb") as f:
        image_paths = [line.decode("utf-8").strip() for line in f.readlines()]
    return image_paths

def get_data_path(data_folder: str, f: str, time: int):
    with open(os.path.join(data_folder, f, f"image_paths.txt"), "rb") as f:
        image_paths = [line.decode("utf-8").strip() for line in f.readlines()]
    return image_paths[time]
